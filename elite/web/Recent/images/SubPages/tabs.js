...
/*
 * function expandcontent(cid, aobject, tabid){ if (disabletablinks==1)
 * aobject.onclick=new Function("return false"); if (document.getElementById &&
 * turntosingle==0){ highlighttab(aobject, tabid); previoustab_id =
 * prev_tab_arr[tabid]; //alert(previoustab_id + " >> "+cid+" >> "+tabid); if
 * (previoustab_id!=""){
 * document.getElementById(previoustab_id).style.display="none"; } if (cid!=""){
 * document.getElementById(cid).style.display="block"; prev_tab_arr[tabid] =
 * cid; } } }
 */
$(document).ready(function() {
	expandcontent("#sc1", $("#hometabs").find("ul li a:first"));
});
function expandcontent(tabid, obj) {
	$("#hometabs").find("ul li a").attr("class", "");
	$(obj).attr("class", "current");
	$("#tabcontentcontainer div.tabcontent").css("display", "none");
	$(tabid).css("display", "");
}
...
/*
 * if (window.addEventListener) window.addEventListener("load", do_onload,
 * false) else if (window.attachEvent) window.attachEvent("onload", do_onload)
